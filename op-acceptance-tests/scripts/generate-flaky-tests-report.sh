...
echo "API URL: https://circleci.com/api/v2/insights/gh/$ORG_NAME/$REPO_NAME/flaky-tests?branch=$BRANCH"
echo "Debug: Making API request with token length: ${#CIRCLE_API_TOKEN}"
...
echo "CSV report generated with $(wc -l < "$OUTPUT_DIR/flaky_tests.csv") lines"
...
<body>
    ...
    <table>
        ...
        $(jq -r '.flaky_tests[] | "<tr><td>\(.times_flaked)</td><td>\(.test_name)</td><td>\(.classname)</td><td>\(.job_name)</td><td>\(.workflow_name)</td><td>\(.workflow_created_at)</td><td>\(.workflow_created_at)</td></tr>"' "$OUTPUT_DIR/flaky_tests.json")
    </table>
</body>
</html>
EOF
...
# Output text report to stdout (top10 only)
echo "Flaky Tests Report for branch $BRANCH"
echo "=========================================="
echo "Total flaky tests: $NUM_TESTS"
echo
jq -r '.flaky_tests | sort_by(.times_flaked) | reverse | .[0:10] | .[] | "# Flakes: \(.times_flaked) | Test: \(.test_name) | Path: \(.classname) | Job: \(.job_name) | Workflow: \(.workflow_name) | First Flaked: \(.workflow_created_at) | Last Flaked: \(.workflow_created_at)"' \
